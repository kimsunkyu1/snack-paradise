...
<!-------------------------------------------------------------------------------------------->	
			...
			<table border="0" cellpadding="5" cellspacing="1" width="710" class="cmfont" bgcolor="#CCCCCC">
				...
				for($i=1;$i<=$n_cart;$i++)
				{
					if($cart[$i])
					{
						...
						if($row['discount14']==0)
						{
							...
							$toprice = number_format($row['price14']*$num);
							$toprice1 = $row['price14']*$num;
						}
						else
						{
							$price = number_format( round($row['price14']*(100-$row['discount14'])/100, -1));
							$toprice = number_format( round($row['price14']*(100-$row['discount14'])/100*$num, -1));
							$toprice1 = round($row['price14']*(100-$row['discount14'])/100*$num, -1);
						}
						...
						echo("<tr bgcolor='#FFFFFF'>
							<td height='60' align='center'>
								<table cellpadding='0' cellspacing='0' width='100%'>
									<tr>
										<td width='60'>
											<a href='product_detail.php?no=$row[no14]'><img src='product/$row[image1]' width='50' height='50' border='0'></a>
										</td>
										<td class='cmfont'>
											<a href='product_detail.php?no=$row[no14]'><font color='#0066CC'>$row[name14]</font></a><br>
											[옵션]</font> $row1[name14] / $row2[name14]
										</td>
									</tr>
								</table>
							</td>
							...
							<td align='center'><font color='#464646'>$toprice</font> 원</td>
						</tr>");

						$total=$total+$toprice1;

						if($row['menu14'] == 1 || $row['menu14'] == 2)
						{
							$baesongbi = 0;
						}
				...
				<?
					...
					$total_nf = number_format($total);
					$baesongbi_nf = number_format($baesongbi);
					$total1 = number_format($total + $baesongbi);
				?>
				<tr>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0" class="cmfont">
							<tr>
								...
								<td align="right" bgcolor="#F0F0F0">
									<font color="#0066CC"><b>총 합계금액</font></b> : 상품대금(<?=$total_nf?>원) + 배송료(<?=$baesongbi_nf?>원) = <font color="#FF3333"><b><?=$total1?> 원</b></font>&nbsp;&nbsp
								</td>
							</tr>
						</table>
					</td>
			...
			{
				$query="select * from member where no14=$cookie_no;";
				$result=mysqli_query($db,$query);
				if(!$result) exit("에러:$query");
				...
				$o_no = $row['no14'];
				$o_name = $row['name14'];
				$o_tel1 = trim(substr($row['tel14'],0,3));
				$o_tel2 = trim(substr($row['tel14'],3,4));
				$o_tel3 = trim(substr($row['tel14'],7,4));
				$o_phone1 = trim(substr($row['phone14'],0,3));
				$o_phone2 = trim(substr($row['phone14'],3,4));
				$o_phone3 = trim(substr($row['phone14'],7,4));
				$o_email = $row['email14'];
				$o_zip = $row['zip14'];
				$o_juso = $row['juso14'];
			}
					...
					<td align="center" width="560">
						...
						<table width="560" border="0" cellpadding="0" cellspacing="0" class="cmfont">
							...
							<tr height="25">
								...
								<td width="20"><b>:</b></td>
								<td width="390">
									<input type="text" name="o_tel1" size="3" maxlength="3" value="<?=$o_tel1?>" class="cmfont1"> -
							...
							<tr height="25">
								...
								<td width="20"><b>:</b></td>
								<td width="390">
									<input type="text" name="o_phone1" size="3" maxlength="3" value="<?=$o_phone1?>" class="cmfont1"> -
